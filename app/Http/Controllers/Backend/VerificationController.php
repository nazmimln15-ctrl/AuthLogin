...
class VerificationController extends Controller
{
    ...
    /**
     * Public endpoint to send OTP to an email address (no auth required).
     * Used during registration flow when the client cannot use session auth.
     */
    public function storePublic(Request $request)
    {
        $email = $request->input('email');
        if (! $email) return response()->json(['success' => false, 'message' => 'Email required'], 422);
    ...
    /**
     * Public update for verifying OTP without authentication.
     */
    public function updatePublic(Request $request, $unique_id)
    {
        $verify = Verification::where('unique_id', $unique_id)->where('status', 'active')->first();
        if (! $verify) return response()->json(['success' => false, 'message' => 'Not found'], 404);
